    ...
    <form className="multi-step-form" onSubmit={onSubmit} aria-describedby="multi-step-form-note">
      ...
      {step === 1 ? (
        <fieldset className="multi-step-form__fieldset">
          ...
          <div className="multi-step-form__chips" role="radiogroup" aria-label="Asset type">
            {assetOptions.map((option) => (
              <label key={option} className={`choice-chip ${form.assetType === option ? "is-selected" : ""}`}><input type="radio" name="assetType" value={option} checked={form.assetType === option} onChange={handleChange} /><span>{option}</span></label>
            ))}
          </div>
          <div className="multi-step-form__grid">
            <label className="field"><span>I need help with</span><select name="intent" value={form.intent} onChange={handleChange}>{intentOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>
            <label className="field"><span>Preferred area / corridor</span><input name="area" value={form.area} onChange={handleChange} placeholder="Mississauga, Vaughan, North York…" /></label>
            <label className="field"><span>Approximate size or budget</span><input name="sizeNeed" value={form.sizeNeed} onChange={handleChange} placeholder="12,000 SF · $18 PSF net · $4M…" /></label>
            <label className="field"><span>Timing</span><select name="timing" value={form.timing} onChange={handleChange}>{timingOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>
          </div>
        ...
        <fieldset className="multi-step-form__fieldset">
          ...
          <div className="multi-step-form__grid">
            <label className="field"><span>Name</span><input name="name" value={form.name} onChange={handleChange} autoComplete="name" required /></label>
            <label className="field"><span>Email</span><input name="email" type="email" value={form.email} onChange={handleChange} autoComplete="email" required /></label>
            <label className="field"><span>Phone</span><input name="phone" value={form.phone} onChange={handleChange} autoComplete="tel" placeholder={SITE.primaryPhone} /></label>
            <label className="field field--full"><span>Anything we should pressure-test?</span><textarea name="message" rows="5" value={form.message} onChange={handleChange} placeholder="NNN exposure, loading type, cap rate targets, zoning constraints, logistics concerns, decision timing…" /></label>
          </div>
