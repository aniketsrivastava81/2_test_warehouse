...
function buildLocalBusinessSchema() {
  return {
    ...
    description: `${SITE.defaultDescription} Coverage includes GTA and Golden Horseshoe commercial real estate markets.`,
  };
}
...
export default function RouteSeo() {
  ...
  const page = useMemo(() => {
    ...
    if (pathname === "/") {
      return {
        title: "GTA and Golden Horseshoe Commercial Real Estate Advisory",
        description: `${SITE.defaultDescription} Built for Toronto, Mississauga, Brampton, Vaughan, Richmond Hill, and the broader Golden Horseshoe.`,
        canonicalPath: "/",
        keywords: ["KOLT Realty", "GTA commercial real estate", "Golden Horseshoe industrial space", "warehouse advisory Toronto", "Mississauga industrial real estate"],
    ...
    if (pathname === "/services") return buildPage("Services", "Tenant, landlord, owner-user, investment, land, and portfolio advisory paths structured to reduce decision risk across the GTA and Golden Horseshoe.", "/services", ["commercial real estate services", "tenant representation GTA", "owner-user acquisition Toronto", "Golden Horseshoe commercial advisory"]);
    if (pathname === "/listings") return buildPage("Listings", "Browse industrial, warehouse, office, retail, and land opportunities framed through fit, corridor logic, and commercial usefulness across the GTA.", "/listings", ["GTA commercial listings", "industrial listings Toronto", "warehouse opportunities GTA"]);
    ...
    if (pathname === "/contact") return buildPage("Contact", "Contact KOLT Realty through the multi-step requirement brief, sample inbox, sample WhatsApp routing, and direct phone details for GTA commercial real estate inquiries.", "/contact", ["contact KOLT Realty", "commercial real estate GTA"]);
    if (pathname === "/markets") return buildPage("Markets", "GTA and Golden Horseshoe markets explained through occupancy, corridor access, labour logic, customer reach, and real commercial usefulness.", "/markets", ["GTA markets", "Toronto commercial markets", "industrial corridors GTA"]);
    ...
    if (pathname.startsWith("/listings/")) {
      ...
      return { title: `${listing.title} in ${listing.location}`, description: `${listing.teaser} ${listing.size} ${listing.category} opportunity in ${listing.location}.`, canonicalPath: `/listings/${listing.slug}`, keywords: [listing.category, listing.location, listing.status, "commercial listing GTA"], structuredData: [["listing-breadcrumbs", buildBreadcrumbSchema([{ name: "Home", path: "/" }, { name: "Listings", path: "/listings" }, { name: listing.title, path: `/listings/${listing.slug}` }])], ["listing-offer", { "@context": "https://schema.org", "@type": "Offer", name: listing.title, category: listing.category, description: listing.teaser, availability: "https://schema.org/InStock", url: absolute(`/listings/${listing.slug}`) }]] };
    }
